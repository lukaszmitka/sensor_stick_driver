...
int gnublin_i2c::receive(unsigned char *RxBuf, int length) {
   ...
   if (read(fd, RxBuf, length) != length)
      return errorMsg("i2c read error! Address: " + numberToString(slave_address) + " dev file: " +
                      devicefile + "\n");
...
int gnublin_module_pca9555::pinMode(int pin, std::string direction) {
   ...
   if (pin >= 0 && pin <= 7) { // Port 0
      ...
      if (i2c.receive(0x06, RxBuf, 1) > 0) { //read the current state
         ...
         if (direction == "out") {
            TxBuf[0] = RxBuf[0] &
                       ~TxBuf[0]; // at output you have to invert the pin you want to set und AND it to change only the pin
...
int gnublin_module_pca9555::digitalWrite(int pin, int value) {
   ...
   if (pin >= 0 && pin <= 7) { // Port 0
      ...
      if (i2c.receive(0x02, RxBuf, 1) > 0) { //read the current state
         ...
         if (value == 0) {
            TxBuf[0] = (RxBuf[0] &
                        ~TxBuf[0]); // at low you have to invert the pin you want to set and do a AND to change only the pin you want
...
int gnublin_module_step::burnNewAddress(int new_address) {
   ...
   if (new_address <= slave_address) {
      ...
      if (((new_ad & 0x1) < (old_ad & 0x1)) | ((new_ad & 0x2) < (old_ad & 0x2)) |
          ((new_ad & 0x4) < (old_ad & 0x4)) | ((new_ad & 0x8) < (old_ad & 0x8))) {
...
int gnublin_module_step::setMotorParam() {
   ...
   buffer[7] = (unsigned char) ((accshape * 0x10) +
                                stepmode);  //AccShape & StepMode 00=Halfstep, 04=1/4µStep, 08=1/8µStep, 0c=1/16µStep
...
* @return Erfolg: 1, Fehler: -1
*/
int gnublin_module_step::setMotorParam(unsigned int newIrun, unsigned int newIhold,
                                       unsigned int newVmax, unsigned int newVmin,
                                       unsigned int newAccShape, unsigned int newStepMode) {
   irun = newIrun;
   ihold = newIhold;
   vmax = newVmax;
   vmin = newVmin;
...
void gnublin_smtp::Send() {
   ...
   std::string encoded_login = base64_encode(
         reinterpret_cast<const unsigned char *>(m_sLogin.c_str()), m_sLogin.size());
   sprintf(SendBuf, "%s\r\n", encoded_login.c_str());
   SendData();
   bAccepted = false;
   do {
   ...
   std::string encoded_password = base64_encode(
         reinterpret_cast<const unsigned char *>(m_sPassword.c_str()), m_sPassword.size());
   ...
   for (FileId = 0; FileId < Attachments.size(); FileId++) {
      ...
      if (TotalSize / 1024 > MSG_SIZE_IN_MB * 1024)
         ...
         for (i = 0; i < FileSize / 54 + 1; i++) {
            ...
            MsgPart ? strcat(SendBuf,
                             base64_encode(reinterpret_cast<const unsigned char *>(FileBuf), res)
                                   .c_str())
                    : strcpy(SendBuf,
                             base64_encode(reinterpret_cast<const unsigned char *>(FileBuf), res)
                                   .c_str());
...
void gnublin_smtp::FormatHeader(char *header) {
   ...
   if (!Attachments.size()) { // no attachments
