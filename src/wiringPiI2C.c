 ...
 * ...this would require to include wiringPi.h, in order to report errors
 *********************************************************************************
 */
...
int wiringPiI2CSetupInterface(const char *device, int devId) {
   ...
   if ((fd = open(device, O_RDWR)) < 0) {
      ...
      return -1;
   } else if (ioctl(fd, I2C_SLAVE, devId) < 0) {
...
int wiringPiI2CSetup(const int devId) {
   /*
    * commented lines could be useful for implementation on
    * various versions of Pi, would require to include wiringPi.h
    */
   //int rev ;
   const char *device;
